param(
    ...
    [string]$LogPath = "F:\egg baseline\DMMR\logs\quick_tgb_sspb_v2_gcnres015_selfloop005_nodrop.log"
)

$repoRoot = Split-Path -Parent $PSScriptRoot
New-Item -ItemType Directory -Force -Path (Split-Path -Parent $LogPath) | Out-Null
New-Item -ItemType Directory -Force -Path "$repoRoot\outputs\quick_tgb_sspb_v2_gcnres015_selfloop005_nodrop" | Out-Null
...
try {
    & $Python "$repoRoot\main.py" `
        ...
        --use_gcn_residual `
        --gcn_alpha_init 0.15 `
        --gcn_learnable_alpha 1 `
        --use_self_loop_prior `
        --self_loop_weight 0.05 `
        --way outputs/quick_tgb_sspb_v2_gcnres015_selfloop005_nodrop `
