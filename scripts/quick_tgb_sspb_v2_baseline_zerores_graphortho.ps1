param(
    ...
    [string]$LogPath = "F:\egg baseline\DMMR\logs\quick_tgb_sspb_v2_baseline_zerores_graphortho.log"
)

$repoRoot = Split-Path -Parent $PSScriptRoot
New-Item -ItemType Directory -Force -Path (Split-Path -Parent $LogPath) | Out-Null
New-Item -ItemType Directory -Force -Path "$repoRoot\outputs\quick_tgb_sspb_v2_baseline_zerores_graphortho" | Out-Null
...
try {
    & $Python "$repoRoot\main.py" `
        ...
        --use_prompt_ortho_loss 0 `
        --way outputs/quick_tgb_sspb_v2_baseline_zerores_graphortho `
