param(
    ...
    [string]$LogPath = "F:\egg baseline\DMMR\logs\quick_tgb_sspb_v2_improved_zerores_graphortho.log"
)

$repoRoot = Split-Path -Parent $PSScriptRoot
New-Item -ItemType Directory -Force -Path (Split-Path -Parent $LogPath) | Out-Null
New-Item -ItemType Directory -Force -Path "$repoRoot\outputs\quick_tgb_sspb_v2_improved_zerores_graphortho" | Out-Null
...
try {
    & $Python "$repoRoot\main.py" `
        ...
        --prompt_dropout 0.0 `
        --use_zero_init_prompt_residual 1 `
        --prompt_fusion_dropout 0.1 `
        --stable_adj_alpha 0.2 `
        --use_prompt_ortho_loss 1 `
        --prompt_ortho_weight 0.01 `
        --way outputs/quick_tgb_sspb_v2_improved_zerores_graphortho `
