param(
    ...
    [string]$LogPath = "F:\egg baseline\DMMR\logs\quick_tgb_sspb_v2_baseline_avgcmp.log"
)

$repoRoot = Split-Path -Parent $PSScriptRoot
New-Item -ItemType Directory -Force -Path (Split-Path -Parent $LogPath) | Out-Null
New-Item -ItemType Directory -Force -Path "$repoRoot\outputs\quick_tgb_sspb_v2_baseline_avgcmp" | Out-Null
...
try {
    & $Python "$repoRoot\main.py" `
        ...
        --prompt_dropout 0.0 `
        --way outputs/quick_tgb_sspb_v2_baseline_avgcmp `
