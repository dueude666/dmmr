...
def new_SSDA(args):#Select Source Domain Algorithm(SSDA)
    ...
    if getattr(args, "use_feature_mixstyle", False) and FeatureMixStyle is not None:
        ...
        print(f"[SSAS] FeatureMixStyle enabled in source-selection stage: p={mixstyle.p}, alpha={mixstyle.alpha}")
    ...
    for i in range(args.max_iter1):
        ...
        save_root = getattr(args, "count_dir", os.path.join(args.output_dir, "count"))
        save_path = os.path.join(save_root, str(trg_subj))
